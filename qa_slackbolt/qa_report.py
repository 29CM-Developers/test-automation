...
def open_modal(ack, body, client):
    ack()
    slack_modal_blocks.modal_block(body, client, feature_name, significant='')


@app.action("feature_check")
def feature_check_open_modal(ack, body, client):
    ack()
    slack_modal_blocks.update_modal_block(body, client, feature_name)
...
def slack_message(ack, body, logger, client):
    ...
    try:
        ...
        if 'TestRail API returned HTTP 400' in str(e):
            error_message = '*Testrail의 plan number를 확인해주세요.*'
        else:
            error_message = str(e)
        slack_modal_blocks.error_modal_block(body, client, feature_name, significant_text, error_message)
