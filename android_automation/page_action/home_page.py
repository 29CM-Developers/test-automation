...
def click_dynamic_gate(wd):
    ...
    for i in range(0, 3):
        try:
            element = aal(wd, '센스있는 선물하기')
            if element.is_displayed():
                aalc(wd, '센스있는 선물하기')
                sleep(3)
                break
            else:
                dynamic_gate = aal(wd, 'dynamic_gate')
...
def check_dynamic_gate_gift_page(wd):
    try:
        ...
        aal(wd, '//span[contains(text(), "선물인가요")]')
...
def save_banner_title(wd):
    ...
    for i in range(0, 3):
        try:
            ...
            banner_title = aal(wd, title_element).text
...
def save_contents_like_count(wd):
    content_like_count = aal(wd, 'home_content_like_count')
    for i in range(0, 3):
...
def click_contents_product(wd):
    aalc(wd, '//XCUIElementTypeOther[@name="home_content_product"]')
    sleep(1)
