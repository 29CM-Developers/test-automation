...
class Search:
    def test_search_popular_brand(self, wd, test_result='PASS', error_texts=[], img_src='', warning_texts=[]):
        ...
        try:
            ...
            # 전체 기준 인기검색어 리스트 호출
            reponse = requests.get('https://search-api.29cm.co.kr/api/v4/keyword/popular?limit=100&brandLimit=30')
            all_popular_data = reponse.json()
            ...
            # 변경된 필터 기준
            reponse = requests.get('https://search-api.29cm.co.kr/api/v4/keyword/popular?limit=100&brandLimit=30&group=female&ageGroup=30to34')
            filter_popular_data = reponse.json()
