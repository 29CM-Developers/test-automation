...
class IOSTestAutomation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.appium.start(
            args=['-p', device_info['ios_manual_port'], '--base-path', '/wd/hub', '--default-capabilities',
                  '{"appium:chromedriverExecutable": "/usr/local/bin"}'])

        # webdriver
        self.wd, self.iOS_cap = getattr(ios_setup, device_info['ios_manual_device'])()
